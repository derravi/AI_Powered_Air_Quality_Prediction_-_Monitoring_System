...
sm = SMOTE(random_state=42)  # UPDATED: added to balance classes
...
print("\nEnter All the Values of yout Enviornment for the Correct Prediction.\n")

def UserInput():
    City = input("Enter the City Name : ")
    Day = int(input("Enter the Date(Like 12,15,29) : "))
    Month = int(input("Enter the Month of Date(Like Jan=1,Fab=2...) : "))
    ...
    #Make the DataFrame of the User input
    user_df = pd.DataFrame([user_input_data])

    #Encode the Catagoricle Columns from User Input.
    user_df['City'] = lb.transform(user_df['City'])

    #Encode the Input Data using the StandarsScaler
    user_df_scaled = std.transform(user_df)

    #Make Prediction for the Both Model
    knn_pred_num = knn.predict(user_df_scaled)[0]
    tree_pred_num = dtree.predict(user_df_scaled)[0]

    #Convert Encode Output into the Origonal Labels.
    ...
    print("\n==========Prediction Result==========\n")
    print(f"\nKNN Model predicts the Enviornment is: {knn_pred_label}({knn_pred_num})")
    print(f"\nDecision Tree Model predicts the Enviornment is: {dtree_pred_label}({dtree_pred_num})")
    print("\n=====================================\n")
...
plt.tight_layout()
plt.savefig("model_accuracy_comparison.png",dpi=400,bbox_inches='tight')
plt.show()
